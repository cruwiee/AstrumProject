...
//   // Загрузка данных о корзине из localStorage
//   useEffect(() => {
//     const savedCart = JSON.parse(localStorage.getItem('cartItems')); 
//     if (savedCart && Array.isArray(savedCart)) {
//       setCartItems(savedCart); // Загружаем корзину из localStorage
//     } else {
//       console.log("Корзина пуста в localStorage, загружаем с сервера");
//       fetchCart();  // Загружаем корзину с сервера, если она пуста
//     }
...
//       console.log("Сохраняем корзину в localStorage:", cartItems);
//       localStorage.setItem('cartItems', JSON.stringify(cartItems));
//     }
//   }, [cartItems]); // Каждый раз, когда корзина изменяется

//   // Проверка и установка user и token
//   useEffect(() => {
...
//   useEffect(() => {
//     if (cartItems.length === 0) {
//       localStorage.removeItem("cartItems"); // Принудительное удаление данных корзины
//     }
//   }, [cartItems]);
...
//     if (!user || !user.userId) return; // Проверяем наличие user
//     try {
//       console.log("Загружаем корзину с сервера...");
//       const cartItemsFromServer = await getCartItems(user.userId);
//       setCartItems(cartItemsFromServer); // Загружаем корзину с сервера
//     } catch (error) {
...
//     setCartItems(updatedCart); // Обновляем корзину на клиенте
//   } catch (error) {
...
//   if (!userId) return;
//   try {
//     await updateCartItem(cartId, quantity); // Обновление на сервере
//     const updatedCart = await getCartItems(user.userId); // Получаем обновленную корзину с сервера
//     setCartItems(updatedCart); // Обновляем состояние корзины на клиенте
//   } catch (error) {
...
//   if (!userId) return;
//   try {
//     await removeFromCart(cartId); // Удаление с сервера
//     const updatedCart = await getCartItems(user.userId); // Получаем обновленную корзину с сервера
//     setCartItems(updatedCart); // Обновляем состояние корзины на клиенте
//   } catch (error) {
...
//   // useEffect для загрузки профиля при изменении token
//   useEffect(() => {
//     if (token) {
//       fetchUserProfile();
//     }
  ...
  const initializeCart = useCallback(async () => {
    if (!user || !user.userId) {
      ...
      try {
        ...
        localStorage.removeItem('cartItems'); // Очищаем некорректные данные
      }
      return;
    }
